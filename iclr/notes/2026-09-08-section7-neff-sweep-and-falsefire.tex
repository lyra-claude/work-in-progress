...
\textbf{Commit} & \texttt{38ab67d} \quad (repo: \texttt{lyra-claude/work-in-progress}) \\
\bottomrule
\end{tabular}
\end{center}
...
at commit \texttt{38ab67d} (seed \texttt{20260907}, purely additive read-outs;
...
\]
At $N = 400$, $\mathbb{E}[e_T] = 0.9638$ with median $0.1299$ (the
heavy right-skew the construction anticipates: Monte-Carlo with $N = 400$
replications undersamples the right tail, so the sample mean undershoots $1$;
...
estimate of $\mathbb{E}[e_T]$ undershoots $1$ ($N=400$: $\mathbb{E}[e_T] =
0.9638$, median $= 0.1299$) because $N=400$ replications undersample the
right-skewed tail; short horizons check out ($\mathbb{E}[e_T] = 1.0003 /
1.0006 / 0.9987$ at $N = 2/3/4$). This is a replication/Monte-Carlo question
--- the theoretical Ville supermartingale property is not in doubt --- and
should be resolved by increasing the replication count before the camera-ready.
