...
\paragraph{Null-model relativity motivates a null-free monitor.}
A sharper form of the objection is that monoculture is not an absolute property but a
\emph{null-model-relative} one.  Jo, Garg, and Raghavan
(\href{https://arxiv.org/abs/2602.24086}{arXiv:2602.24086}) formalize excess inter-model
agreement only relative to a chosen difficulty null, and climb an IRT-style ladder of
increasingly expressive nulls; their Theorem~3 shows that the item-averaged residual
inter-model covariance tends to zero \emph{once the difficulty null is made expressive enough
to absorb the observed structure}---that is, as the discrepancy $D(P, Q^*_K)$ between the data
law $P$ and the best-fitting null $Q^*_K$ goes to zero, \emph{not} as any item or model count
grows.  The vanishing is therefore hostage to a modeling commitment: any correlation verdict
is only as meaningful as the difficulty null one agrees to condition on.  This inverts into an
argument \emph{for} our construction rather than against it.  The cross-item statistic
$V = W_i^s W_j^t$ posits no item-difficulty null at all---it secures a marginals-free baseline
$\mathbb{E}[V] = ab$ purely from item independence---so the hypothesis of their theorem, the
existence and fit of a rich difficulty null, never enters our estimand.  Their result names
precisely the dependence on a modeling choice that a null-model-free monitor is engineered to
escape.
